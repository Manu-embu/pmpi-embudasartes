...
async function iniciarMapa() {
  const [
    ...
    buscarJson("./dados/equipamentos/catalogo-camadas.json", {camadas:[]})
  ]);
  ...
  for (const definicao of (catalogoUrbanos.camadas || [])) {
    const geojson = await buscarJson(`./${definicao.arquivo}`, {type:"FeatureCollection",features:[]});
    ...
    (geojson.features || []).forEach(feature => {
      ...
      L.circleMarker([lat, lng], {
        ...
        color: "#ffffff",
        weight: 1.5,
        fillColor: definicao.cor || "#555555",
        fillOpacity: 0.95
      }).addTo(grupo).bindPopup(`
        <div class="map-popup">
          <span class="map-popup-badge">${definicao.categoria} • ${definicao.subcategoria}</span>
          <h3>${props.nome || "Equipamento"}</h3>
          <p><strong>Endereço:</strong> ${props.endereco || "Não informado"}</p>
          <p><strong>Bairro informado:</strong> ${props.bairro_informado || "Não informado"}</p>
          <p><strong>Referência da base:</strong> ${props.ano_referencia || definicao.ano_referencia || 2021}</p>
    ...
    // O usuário solicitou que todos os equipamentos apareçam no mapa.
    grupo.addTo(mapa);
    camadasUrbanas[`${definicao.categoria} — ${definicao.nome} (${definicao.quantidade})`] = grupo;
  }
  ...
  L.control.layers(bases, overlays, { collapsed:true }).addTo(mapa);
  ...
  document.querySelector("#mapa-resumo").innerHTML = `
    <strong>Atlas com cartografia oficial, ${statusResumo.validado + statusResumo["a-validar"] + statusResumo.revisar} unidades de Educação Infantil e ${totalUrbanos} equipamentos urbanos.</strong>
    <span>
  ...
  if (catalog) {
    ...
    const urbanosCards = (catalogoUrbanos.camadas || []).map(definicao => `
      <div class="atlas-layer-item">
        <span style="background:${definicao.cor || "#666"}"></span>
        <strong>${definicao.nome}</strong>
        <small>${definicao.quantidade} registro(s) • referência ${definicao.ano_referencia}</small>
      </div>`).join("");
    const territoriaisCards = Object.entries(counts).map(([category, count]) => `
      <div class="atlas-layer-item">
        <span style="background:${cores[category] || "#666"}"></span>
        <strong>${category}</strong>
        <small>${count} registro(s) inventariado(s)</small>
      </div>`).join("");
    catalog.innerHTML = urbanosCards + territoriaisCards;
  }
}
